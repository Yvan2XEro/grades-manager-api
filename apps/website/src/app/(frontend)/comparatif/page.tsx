			...
			<section className="mx-auto max-w-[86rem] px-6 py-14 lg:px-10 lg:py-20">
				<div className="overflow-x-auto rounded-xl border border-tk-border">
					<table className="w-full border-collapse">
						<thead>
							<tr>
								<th className="bg-tk-dark px-4 py-3.5 text-left font-body font-semibold text-[0.82rem] text-tk-on-dark sm:px-5">
									{en ? "Criterion" : "Critère"}
								</th>
								<th className="bg-tk-accent px-4 py-3.5 text-left font-body font-semibold text-[0.82rem] text-white sm:px-5">
									QR Code OnReceipt
								</th>
								<th className="bg-tk-primary px-4 py-3.5 text-left font-body font-semibold text-[0.82rem] text-tk-on-primary sm:px-5">
									TKAMS
								</th>
							</tr>
						</thead>
							...
							{COMPARISON.map((row, i) => {
								...
								return (
									...
									>
										<th
											scope="row"
											className="px-4 py-3.5 text-left align-top font-body font-semibold text-[0.85rem] text-tk-ink sm:px-5"
										>
											{row.criterion}
										</th>
										<td
											className={`px-4 py-3.5 align-top font-body text-[0.85rem] leading-relaxed sm:px-5 ${
				...
				<div className="mt-8 grid gap-5 lg:grid-cols-2">
					<div className="rounded-xl border border-tk-border bg-tk-bg-deep p-6">
						...
						<p className="mt-4 font-body text-[0.82rem] text-tk-muted leading-relaxed">
							{en
					...
					<div className="flex flex-col justify-between gap-5 rounded-xl bg-tk-dark p-6 text-tk-on-dark">
						<div>
							...
							<p className="mt-3 font-body text-[0.9rem] text-tk-on-dark-soft leading-relaxed">
								{en
						...
						<div className="flex flex-wrap gap-3">
							<Link
								href="/contact"
								className="rounded-md bg-tk-primary px-5 py-3 font-body font-semibold text-[0.9rem] text-tk-on-primary transition-colors hover:bg-tk-primary-bright hover:text-tk-dark"
							>
								{en ? "Request a demo" : "Demander une démo"}
							</Link>
							<Link
								href="/tarifs"
								className="rounded-md border border-white/30 px-5 py-3 font-body font-semibold text-[0.9rem] text-tk-on-dark transition-colors hover:bg-white/10"
							>
