		...
		<main className="tk-dotgrid bg-tk-bg pt-[var(--tk-header-h)]">
				...
				<div className="mx-auto max-w-[86rem] px-6 py-14 lg:px-10 lg:py-20">
					<div className="grid gap-10 lg:grid-cols-[1.1fr_minmax(0,26rem)] lg:items-start lg:gap-14">
						<div>
							...
							<h1 className="tk-display tk-gradient-text mt-4 max-w-[20ch]">
								{en
									? "Two lines, one tier, no surprises."
									: "Deux lignes, un palier, aucune surprise."}
							</h1>
							<p className="mt-5 max-w-[56ch] font-body text-[length:var(--tk-text-lead)] text-tk-ink-2 leading-relaxed">
								{en
			...
			<section className="mx-auto max-w-[86rem] px-6 py-14 lg:px-10 lg:py-20">
						...
						<thead>
							<tr className="bg-tk-dark">
								{[
									...
									<th
										key={h}
										className="px-4 py-3.5 text-left font-body font-semibold text-[length:var(--tk-text-sm)] text-tk-on-dark sm:px-5"
									>
										{h}
									</th>
								))}
							...
							{lines.map((l) => (
								...
								>
									...
									<td className="px-4 py-4 align-top sm:px-5">
										<span className="block font-bold font-display text-[length:var(--tk-text-lead)] text-tk-primary-deep tabular-nums">
											{l.amount}
										</span>
										{l.sub ? (
											<span className="block font-body text-[length:var(--tk-text-xs)] text-tk-muted">
												{l.sub}
											</span>
										) : null}
									</td>
									<td className="max-w-[34rem] px-4 py-4 align-top font-body text-[length:var(--tk-text-sm)] text-tk-ink-2 leading-relaxed sm:px-5">
										{l.pays}
									</td>
									<td className="px-4 py-4 align-top font-body text-[length:var(--tk-text-sm)] text-tk-muted sm:px-5">
										{l.freq}
									</td>
								</tr>
							))}
				...
				<div className="mt-6 grid gap-5 lg:grid-cols-[1.3fr_1fr]">
					<div className="rounded-xl bg-tk-dark p-6 text-tk-on-dark lg:p-7">
						...
						<div className="mt-5 grid gap-5 sm:grid-cols-3">
							{STUDENT_BANDS.map((b, i) => (
								<div key={b.label}>
									...
									<p className="mt-1.5 font-body text-[length:var(--tk-text-sm)] text-tk-on-dark-muted leading-snug">
										{en
											? [
						...
						<p className="mt-5 border-white/12 border-t pt-4 font-body text-[length:var(--tk-text-sm)] text-tk-on-dark-soft leading-relaxed">
							{en
					...
					<div className="rounded-xl border border-tk-primary/40 bg-tk-primary-soft p-6 lg:p-7">
						...
						<p className="mt-3 font-body font-semibold text-[length:var(--tk-text-body)] text-tk-ink">
			...
			<section className="border-tk-border border-y bg-tk-bg-deep">
				<div className="mx-auto max-w-[86rem] px-6 py-14 lg:px-10 lg:py-20">
					...
					<p className="mt-4 max-w-[54ch] font-body text-[length:var(--tk-text-body)] text-tk-ink-2 leading-relaxed">
						{en
					...
					<div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
						{simulations.map((s) => (
							...
							>
								<p className="font-code text-[length:var(--tk-text-xs)] text-tk-muted uppercase tracking-[0.12em]">
								...
								<p className="mt-3 border-tk-border border-t pt-3 font-body text-[length:var(--tk-text-sm)] text-tk-ink-2 leading-relaxed">
					...
					<div className="mt-6 rounded-xl bg-tk-dark p-6 text-tk-on-dark lg:p-7">
						...
						<p className="mt-3 max-w-[72ch] font-body text-[length:var(--tk-text-body)] leading-relaxed">
							{en
						...
						<Link
							href="/onreceipt"
							className="mt-5 inline-block font-body font-semibold text-[length:var(--tk-text-body)] text-tk-primary-bright underline-offset-4 hover:underline"
						>
			...
			<section className="mx-auto max-w-[86rem] px-6 py-14 lg:px-10 lg:py-20">
				<div className="grid gap-10 lg:grid-cols-2 lg:gap-14">
					<div>
						...
						<p className="mt-3 font-body text-[length:var(--tk-text-sm)] text-tk-muted leading-relaxed">
							{en
						...
						<p className="mt-4 rounded-lg border border-tk-primary/30 bg-tk-primary-soft p-4 font-body text-[length:var(--tk-text-sm)] text-tk-ink-2 leading-relaxed">
							{en
							...
							{OPTIONS.map((o) => (
								<li key={o.name} className="p-5">
									<div className="flex flex-wrap items-baseline justify-between gap-3">
										<span className="font-body font-semibold text-[length:var(--tk-text-body)] text-tk-ink">
											{o.name}
										</span>
										<span className="font-bold font-display text-[length:var(--tk-text-body)] text-tk-eyebrow tabular-nums">
											{o.price} FCFA
										</span>
									</div>
									<p className="mt-1.5 font-body text-[length:var(--tk-text-sm)] text-tk-muted leading-relaxed">
										{o.note}
									</p>
								</li>
							))}
					...
					>
						{COST_BREAKDOWN.map((c, i) => (
							<div
								key={c.label}
								style={{ width: `${c.pct}%` }}
								className={`px-3 py-3 font-body font-semibold text-[length:var(--tk-text-sm)] ${
									[
							...
							>
								...
								<span className="block truncate font-medium text-[length:var(--tk-text-xs)] opacity-90">
									{c.label}
								</span>
							</div>
						))}
					...
					<dl className="mt-5 grid gap-x-8 gap-y-4 sm:grid-cols-2 lg:grid-cols-3">
						{COST_BREAKDOWN.map((c) => (
							<div key={c.label} className="border-tk-border border-t pt-3">
								...
								<dd className="mt-1 font-body text-[length:var(--tk-text-sm)] text-tk-muted leading-relaxed">
									{c.detail}
								</dd>
							</div>
						))}
					...
					<div className="mt-8 grid gap-4 sm:grid-cols-3">
						{[
							...
							>
								...
								<p className="mt-2.5 font-body text-[length:var(--tk-text-sm)] text-tk-ink-2 leading-relaxed">
									{s.d}
								</p>
							</div>
						))}
			...
			{/*
			 * Between how the price is built and what it excludes — the two halves
			 * of the same promise, separated by a breath.
			 */}
			<PhotoBand
				src="/images/web/etudiante-livres-band.webp"
				alt={
					en
						? "A student on her campus, holding her books"
						: "Une étudiante sur son campus, ses livres à la main"
				}
				caption={
					en
						? "233 FCFA a month per student. Here is what it does not buy."
						: "233 FCFA par mois et par étudiant. Voici ce que cela ne paie pas."
				}
				align="right"
			/>
			{/* Not included */}
			<section className="relative overflow-hidden bg-tk-dark text-tk-on-dark">
				...
				<div className="relative mx-auto max-w-[86rem] px-6 py-14 lg:px-10 lg:py-20">
					...
					<h2 className="tk-headline mt-4 max-w-[24ch]">
						{en ? "What is not included" : "Ce qui n'est pas inclus"}
					</h2>

					<p className="mt-4 max-w-[62ch] font-body text-[length:var(--tk-text-body)] text-tk-on-dark-soft leading-relaxed">
						{en
							...
							return (
								...
								>
									<div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1.5">
										<h3 className="font-body font-semibold text-[length:var(--tk-text-body)] text-tk-on-dark leading-snug">
											{c.item}
										</h3>
										<span className="whitespace-nowrap rounded-full border border-tk-accent/40 bg-tk-accent/10 px-2.5 py-0.5 font-body font-medium text-[length:var(--tk-text-xs)] text-tk-accent">
											{c.owner}
										</span>
									</div>
									<p className="mt-2.5 max-w-[52ch] font-body text-[length:var(--tk-text-sm)] text-tk-on-dark-muted leading-relaxed">
										{c.note}
									</p>
								</li>
							);
					...
					<Link
						...
						className="mt-6 inline-block font-body font-medium text-[length:var(--tk-text-body)] text-tk-on-dark-soft underline-offset-4 hover:text-tk-on-dark hover:underline"
					>
						{en
