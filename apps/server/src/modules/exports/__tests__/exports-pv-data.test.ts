...
async function setupPVFixture(opts?: {
	...
	// Create a single shared teacher profile to avoid costly auth.api.createUser() calls
	const teacherProfile = await createDomainUser();
	...
	const course1a = await createCourse({
		...
		defaultTeacher: teacherProfile.id,
	});
	...
	// Create student (createStudent already inserts an enrollment)
...
// Each test creates a heavy fixture; allow 30s per test
const TEST_TIMEOUT = 30_000;

describe("exports.getPVData", () => {
	it(
		"returns correct UE structure with coefficient-weighted averages",
		async () => {
			const {
				academicYear,
				klass,
	...
	it(
		"UE with failed average gives 0 credits and NON ACQUIS decision",
		async () => {
			const {
				academicYear,
				klass,
	...
	it(
		"incomplete UE (missing grades) gives credits=0 and Inc decision",
		async () => {
			const {
				academicYear,
				klass,
	...
	it(
		"global success rate is correct",
		async () => {
			const {
				academicYear,
				klass,
			...
			// Student 2 (createStudent already inserts an enrollment)
