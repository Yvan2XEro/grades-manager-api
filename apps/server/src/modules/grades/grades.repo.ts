...
export async function listByExam(opts: {
	examId: string;
	cursor?: string;
	limit?: number;
}) {
	...
	const items = await db
		...
		.where(
			and(
				eq(schema.grades.exam, opts.examId),
...
export async function listByStudent(opts: {
	studentId: string;
	cursor?: string;
	limit?: number;
}) {
	...
	const items = await db
		...
		.where(
			and(
				eq(schema.grades.student, opts.studentId),
				opts.cursor ? gt(schema.grades.id, opts.cursor) : undefined,
			),
		)
		.orderBy(schema.grades.id)
...
export async function listByClassCourse(opts: {
