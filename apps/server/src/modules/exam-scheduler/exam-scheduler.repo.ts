...
export async function listRuns(filters: HistoryFilters, institutionId: string) {
	...
	const conditions = [
		filters.facultyId
			? eq(schema.examScheduleRuns.facultyId, filters.facultyId)
			: undefined,
	...
	const rows = await db
		.select({
			...
			facultyId: schema.examScheduleRuns.facultyId,
		...
		.leftJoin(
			schema.faculties,
			eq(schema.faculties.id, schema.examScheduleRuns.facultyId),
		)
		.leftJoin(
