...
export async function queueNotification(data: Omit<schema.NewNotification, "status" | "createdAt">) {
...
export async function sendPending(limit = 25) {
        ...
        for (const notification of pending) {
                const updated = await repo.updateStatus(notification.id, "sent", { sentAt: new Date() });
