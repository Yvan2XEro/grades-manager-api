...
export const workflowsRouter = router({
        validateGrades: adminProcedure
                ...
                .mutation(({ input }) => service.validateGrades(input.examId, input.approverId)),
        enrollmentWindow: adminProcedure
                .input(enrollmentWindowSchema)
                .mutation(({ input }) =>
                        service.toggleEnrollmentWindow(input.classId, input.academicYearId, input.action),
                ),
        enrollmentWindows: protectedProcedure.query(() => service.listEnrollmentWindows()),
