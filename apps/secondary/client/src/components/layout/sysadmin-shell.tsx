...
function SysAdminNav({ onNavigate }: NavProps) {
		...
		<>
			...
			<div className="flex items-center gap-2.5 border-zinc-800 border-b px-4 py-4">
				<div className="flex h-7 w-7 flex-shrink-0 items-center justify-center overflow-hidden rounded bg-primary/10">
					<img src="/logo.png" alt="TKAMS" className="h-5 w-5 object-contain" />
				</div>
				<div>
			...
			{/* Nav */}
			<SysAdminNav onNavigate={() => setMobileOpen(false)} />
	...
	return (
		<BreadcrumbsProvider>
			<div className="flex h-svh overflow-hidden bg-background">
				...
				<aside className="hidden w-56 flex-shrink-0 flex-col bg-zinc-950 text-zinc-100 md:flex">
					<SidebarContent />
				</aside>
				...
				<Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
					<SheetContent
						side="left"
						className="w-56 gap-0 bg-zinc-950 p-0 text-zinc-100"
					>
				...
				<main className="flex min-h-0 flex-1 flex-col overflow-hidden">
					{/* Top bar */}
					<div className="flex flex-shrink-0 items-center gap-3 border-b bg-background/95 px-3 py-3 backdrop-blur sm:px-6">
						{/* Hamburger — mobile only */}
						<button
							type="button"
							onClick={() => setMobileOpen(true)}
							className="flex h-8 w-8 items-center justify-center rounded-md border border-border text-muted-foreground transition-colors hover:bg-muted md:hidden"
						...
						<span className="hidden rounded bg-primary/10 px-2 py-0.5 font-medium text-primary text-xs sm:inline">
